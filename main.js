...
ipcMain.handle('animation-export-reference', async (_, assetId, stateName) => {
  try {
    ...
    } = require('./src/pet/animation-workflow');
    const workflow = resolveAnimationWorkflow(assetId, stateName);
    const result = assetId === 'classic-cat-overlay'
      ? await exportClassicCatReference(workflow)
      : await exportSheetStateReference(workflow);
