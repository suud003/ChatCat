...
async function init() {
  ...
  window.__debugPlayCatAnimation = (stateName) => {
    console.log('[Renderer] __debugPlayCatAnimation:', stateName);
    if (!stateName) return false;
    if (typeof activeCharacter?.playState === 'function') {
      return activeCharacter.playState(stateName) === true;
    }
    if (typeof activeCharacter?._playOverlayState === 'function') {
      activeCharacter._playOverlayState(stateName);
      return true;
    }
    if (stateName === 'drowsy') {
      activeCharacter?.triggerIntent?.('sleepy');
...
function setupSettingsPanel(aiService, apiPresets, chatUI) {
  ...
  const animationEditorPlayLiveBtn = document.getElementById('animation-editor-play-live-btn');
  ...
  animationEditorPlayLiveBtn?.addEventListener('click', () => {
    if (!ENABLE_DEV_DEBUG_TOOLS) return;
    const selected = getSelectedAnimationState();
    if (!selected?.stateName) return;
    try {
      const played = window.__debugPlayCatAnimation?.(selected.stateName);
      if (animationEditorStatus) {
        animationEditorStatus.textContent = played
          ? `已在现实画面中播放: ${selected.stateName}`
          : `当前角色不支持直接播放: ${selected.stateName}`;
      }
      setTimeout(() => { refreshAnimationEditor(); }, 80);
    } catch (err) {
      if (animationEditorStatus) animationEditorStatus.textContent = `播放当前动画失败: ${err.message}`;
    }
  });
